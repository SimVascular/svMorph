...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--target-R", type=float, default=0.15, help="Target lumen radius [cm]")
    parser.add_argument(
        "--stenosis-length", type=float, default=0.5,
        help="Outer annular cutoff r_max [cm]",
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info(f"Initial radius = {current_R:.4f} cm, target = {args.target_R:.4f} cm")
    ...
    iteration = 0
    t0 = time.time()
    while current_R > args.target_R + 2e-3:
